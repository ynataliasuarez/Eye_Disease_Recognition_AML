...
class NumpyDataGenerator:
        ...
        with open(self.csv_testing_path) as csvDataFile:
            ...
            for row in csv_reader:
                ...
                if column_id != "ID":
                    ...
                    # load first the image from the folder
                    try:
                        eye_image = os.path.join(self.testing_path, column_id)
                        image = cv2.imread(eye_image)
                        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                    except:
                        continue
    ...
    def npy_training_files_split(self, split_number, file_name_training, file_name_training_labels, file_name_testing,
        ...
        with open(self.csv_path) as csvDataFile:
            ...
            for row in csv_reader:
                ...
                # just discard the first row
                if column_id != "ID":
                    self.logger.debug("Processing image: " +self.training_path + column_id.split('.')[0] + '_pretty.png')
                    # load first the image from the folder
                    try:
                        
                        image = cv2.resize(cv2.imread(os.path.join(self.training_path, column_id.split('.')[0] + '_pretty.png')), (224,224))
                        print(image.shape)
    ...
    def npy_training_files_split_all(self, split_number, file_name_training, file_name_training_labels,
                                     file_name_testing,
                                     file_name_testing_labels, include_augmented=False):
        split_factor = 10820
        training = []
        training_labels = []
        training_2 = []
            ...
            with open(self.csv_augmented_path) as csvDataFile:
                ...
                for row in csv_reader:
                    ...
                    if column_id != "ID":
                        self.logger.debug("Processing image: " + column_id.split('.')[0] + '_pretty.png')
                        # load first the image from the folder
                        eye_image = os.path.join(self.augmented_path, column_id.split('.')[0] + '_pretty.png')
                        image = cv2.imread(eye_image)
                        image = cv2.resize(image, [224,224,3])
                        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
...
def main(argv):
    ...
    training_file = r'/media/user_home2/ynsuarez/AML/Proyecti/ODIR/ocular-disease-intelligent-recognition-deep-learning/ground_truth/testing_default_value.csv'
    ...
    generator.npy_training_files_split(1000, 'odir_training_seg',
                                    'odir_training_labels_seg', 'odir_testing_seg', 'odir_testing_labels_seg')
    ...
    generator.npy_training_files_split_all(400, 'odir_training_seg' + '_' + str(image_width),
                                            'odir_training_labels_seg' + '_' + str(image_width),
                                            'odir_testing_seg' + '_' + str(image_width),
                                            'odir_testing_labels_seg' + '_' + str(image_width),
                                            False)
