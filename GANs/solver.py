...
class Solver(object):
    ...
    def __init__(self, celeba_loader, rafd_loader,retina_loader, config):
        """Initialize configurations."""

        # Data loader.
        self.celeba_loader = celeba_loader
        self.rafd_loader = rafd_loader
        self.retina_loader = retina_loader
    ...
    def build_model(self):
        ...
        if self.dataset in ['CelebA', 'RaFD','Retina']:
    ...
    def plot_loss(d_losses, g_losses, save_dir, num_iters, save=True, show=False):
        fig, ax = plt.subplots()
        ax.set_xlim(0, num_iters)
        ax.set_ylim(0, max(np.max(g_losses), np.max(d_losses))*1.1)
        plt.xlabel('Epoch No.')
        plt.ylabel('Loss values')
        plt.plot(d_losses, color='plum', label='Discriminator loss')
        plt.plot(g_losses, color='cadetblue', label='Generator loss')
        plt.legend()
    
        if save:
            if not os.path.exists(save_dir):
                os.mkdir(save_dir)
            save_fn = os.path.join(save_dir,'losses.png')
            plt.savefig(save_fn)
        if show:
            plt.show()
        else:
    ...
    def log_loss(d_losses, g_losses, num,directory):
        fieldnames = ['Iter', 'Disc.Loss', 'Gen.Loss']
        save_items = {fieldnames[0]: num, fieldnames[1]: d_losses[-1], fieldnames[2]: g_losses[-1]}
        loss_file = os.path.join(directory, 'loss_log.csv')
    ...
    def train(self):
        ...
        if self.dataset == 'CelebA':
            ...
            data_loader = self.retina_loader
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i+1) % self.log_step == 0:
                ...
                d_losses.append(np.mean(d_losses_epoch))
                g_losses.append(np.mean(g_losses_epoch))
        ...
        if i == self.num_iters: 
            plot_loss(d_losses, g_losses, self.num_iters,save_dir=self.result_dir)
            log_loss(d_losses, g_losses,self.num_iters,self.result_dir )
